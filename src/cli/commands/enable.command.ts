...
/**
 * Enable a provider (exported for testing)
 */
export async function enableProvider(
  ...
  settings.ccai.disabled = false;
  await writeJsonFile(settingsPath, settings, context);

  if (!context?.isDryRun()) {
    logger.success(`Provider ${logger.provider(provider)} enabled`);
    ...
    logger.section("Provider Enabled");
    logger.info(`Command available: ${logger.code(`/ccai-${provider}`)}`);
...
export function createEnableCommand(): Command {
  return new Command("enable")
    ...
    .action(async (provider: string) => {
      try {
        await enableProvider(provider, { skipUpdate: false });
      } catch (error) {
        logger.error(
